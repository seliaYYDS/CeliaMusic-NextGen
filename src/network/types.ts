...
export type KugouSongSearchResult = {
  id: string;
  hash: string;
...
export type KugouPagedResult<T> = {
  items: T[];
  total: number | null;
  limit: number;
  offset: number;
...
export type KugouArtistSummary = {
  id: string;
  name: string;
  avatarUrl: string | null;
  briefDesc: string | null;
...
export type KugouArtistDetail = KugouArtistSummary & {
  coverUrl: string | null;
  description: string | null;
  alias: string[];
};

export type KugouAlbumSummary = {
  id: string;
  name: string;
  artistName: string | null;
  artworkUrl: string | null;
...
export type KugouAlbumDetail = KugouAlbumSummary & {
  description: string | null;
  company: string | null;
  type: string | null;
...
export type KugouPlaylistSummary = {
  id: number;
  collectionId: string;
  name: string;
  description: string | null;
...
export type KugouSongLyrics = NeteaseSongLyrics;

export type KugouResolvedTrack = {
  detail: KugouSongDetail;
  stream: KugouSongStream;
  fallbackStreams: KugouSongStream[];
  lyrics: KugouSongLyrics | null;
  availability: {
    success: boolean;
    message: string | null;
  } | null;
...
export type KugouQrLoginSession = {
  key: string;
  qrUrl: string;
  qrImage: string | null;
};

export type KugouQrLoginStatus = {
  code: 0 | 1 | 2 | 3 | 4 | null;
  message: string | null;
  cookie: string | null;
};

export type KugouAccountProfile = {
  userId: number;
  nickname: string;
  avatarUrl: string | null;
  backgroundUrl: string | null;
